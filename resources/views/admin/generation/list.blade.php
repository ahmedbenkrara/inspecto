...
@section('topnav')
  <x-admin-top-nav title="Générations" />
@endsection
      ...
      <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
        ...
        <div class="flex-auto px-0 pt-0 pb-2">
          <div class="p-0 overflow-x-auto">
            <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
              <thead class="align-bottom">
                <tr>
                  ...
                  <th class="px-6 py-3 font-bold text-left uppercase align-middle bg-transparent border-b border-gray-200 shadow-none text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Génération</th>
              ...
              <tbody>
                @php 
                  $i = 1;
                @endphp
                @foreach($generations as $key => $item)
                  @if($i == 5)
                    @php 
                      $i = 1;
                    @endphp
                ...
                <tr>
                  ...
                  <td class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                    <form action="{{ route('generations.delete', $item->id) }}" method="post" onsubmit="return confirm('Etes-vous sûr de vouloir supprimer cette génération ?')">
                      @csrf
                      @method('DELETE')
                      <a href="/generations/edit/{{$item->id}}" class="text-xs font-semibold leading-tight text-slate-400 mx-2">Modifier</a>
            ...
            {{ $generations->links('vendor.pagination.default') }}
          </div>
        </div>
      </div>
    </div>
